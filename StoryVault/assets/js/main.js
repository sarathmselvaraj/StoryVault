...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const getPortalUrlForUser = (user) => user?.role === 'admin' ? 'admin-dashboard.html' : 'dashboard.html';
    ...
    const buildProfileDropdown = (user, { mobile = false } = {}) => {
        const portalUrl = getPortalUrlForUser(user);
        const firstName = (user.name || 'Reader').trim().split(/\s+/)[0];
        const fullName = user.name || 'StoryVault Reader';
        const email = user.email || 'Signed-in account';
        const avatar = user.avatar || 'assets/images/avatar-member.svg';
        const roleLabel = user.role === 'admin' ? 'Administrator' : 'Reader Member';
        const profileLabel = user.role === 'admin' ? 'Admin Dashboard' : 'Reader Dashboard';
        const planLabel = user.plan || user.title || (user.role === 'admin' ? 'StoryVault Admin' : 'Reader Member');
        const providerLabel = user.provider ? `${String(user.provider).charAt(0).toUpperCase()}${String(user.provider).slice(1)} Sign In` : '';
        const safe = {
            firstName: escapeProfileText(firstName),
            fullName: escapeProfileText(fullName),
            email: escapeProfileText(email),
            avatar: escapeProfileText(avatar),
            roleLabel: escapeProfileText(roleLabel),
            profileLabel: escapeProfileText(profileLabel),
            planLabel: escapeProfileText(planLabel),
            providerLabel: escapeProfileText(providerLabel),
            phone: escapeProfileText(user.phone || ''),
            city: escapeProfileText(user.city || '')
        };

        const wrapper = document.createElement('div');
        wrapper.className = mobile ? 'dropdown mobile-profile-menu d-lg-none' : 'dropdown nav-profile-menu ms-lg-1';

        const button = document.createElement('button');
        button.type = 'button';
        button.className = mobile ? 'mobile-profile-quick' : 'nav-profile-btn';
        button.setAttribute('data-bs-toggle', 'dropdown');
        button.setAttribute('data-bs-auto-close', 'outside');
        button.setAttribute('aria-expanded', 'false');
        button.setAttribute('aria-label', `Open profile menu for ${fullName}`);
        button.title = `Profile: ${fullName}`;
        button.innerHTML = mobile
            ? `<img src="${safe.avatar}" alt="${safe.fullName}">`
            : `<img class="nav-profile-avatar" src="${safe.avatar}" alt="${safe.fullName}">
               <span class="nav-profile-copy"><small>${safe.profileLabel}</small><strong>${safe.firstName}</strong></span>
               <i class="fa-solid fa-chevron-down nav-profile-arrow" aria-hidden="true"></i>`;

        const menu = document.createElement('div');
        menu.className = 'dropdown-menu dropdown-menu-end profile-dropdown-menu';
        menu.innerHTML = `
            <div class="profile-dropdown-header">
                <img class="profile-dropdown-avatar" src="${safe.avatar}" alt="${safe.fullName}">
            ...
            <div class="profile-dropdown-meta">
                <div class="profile-dropdown-meta-row">
                    <span><i class="fa-solid fa-id-badge"></i> Account</span>
                    <strong>${safe.roleLabel}</strong>
                </div>
                <div class="profile-dropdown-meta-row">
                    <span><i class="fa-solid fa-bookmark"></i> ${user.role === 'admin' ? 'Access' : 'Plan'}</span>
                    <strong>${safe.planLabel}</strong>
                </div>
                ${safe.providerLabel ? `<div class="profile-dropdown-meta-row"><span><i class="fa-solid fa-link"></i> Sign-in</span><strong>${safe.providerLabel}</strong></div>` : ''}
                ${safe.phone ? `<div class="profile-dropdown-meta-row"><span><i class="fa-solid fa-phone"></i> Phone</span><strong>${safe.phone}</strong></div>` : ''}
                ${safe.city ? `<div class="profile-dropdown-meta-row"><span><i class="fa-solid fa-location-dot"></i> City</span><strong>${safe.city}</strong></div>` : ''}
            </div>
            <div class="profile-dropdown-actions">
                <a class="profile-dropdown-action" href="${portalUrl}"><i class="fa-solid fa-gauge-high"></i><span>Open ${safe.profileLabel}</span></a>
                <button class="profile-dropdown-action profile-dropdown-logout" type="button"><i class="fa-solid fa-right-from-bracket"></i><span>Logout</span></button>
            </div>`;

        menu.querySelector('.profile-dropdown-logout')?.addEventListener('click', () => window.logoutUser());
        wrapper.append(button, menu);
        return wrapper;
    };

    // Keep the signed-in identity visible across every public page. Clicking the
    // profile control opens a details menu with dashboard access and logout.
    window.syncAuthenticatedNavigation = () => {
        ...
        document.querySelectorAll('[data-dashboard-nav]').forEach((link) => {
            link.href = portalUrl;
            link.setAttribute('aria-label', user.role === 'admin' ? 'Open Admin Dashboard' : 'Open Reader Dashboard');
        });

        document.querySelectorAll('.main-navbar .nav-actions a[href="login.html"]').forEach((link) => {
            link.replaceWith(buildProfileDropdown(user));
        });

        // Mobile gets the same profile details + logout from the compact avatar.
        document.querySelectorAll('.main-navbar .navbar').forEach((navbar) => {
            if (navbar.querySelector('.mobile-profile-menu')) return;
            const toggler = navbar.querySelector('.navbar-toggler');
            if (!toggler) return;
            navbar.insertBefore(buildProfileDropdown(user, { mobile: true }), toggler);
        });
        ...
        document.querySelectorAll('.footer-main a[href^="login.html"]').forEach((link) => {
            ...
            if (!/member sign in|admin sign in/i.test(text)) return;
            link.href = portalUrl;
    ...
    // 2. Dashboard Demo Access
    // Dashboards are intentionally public demo pages in this static template.
    // They use example profiles/sample browser data and never require authentication.
    window.protectPortalAccess = () => true;
    protectPortalAccess();
    ...
    window.setLoginRole = (role) => {
        ...
        const socialBlock = document.querySelector('.social-login-block');
        ...
        if (socialBlock) {
            socialBlock.style.display = role === 'admin' ? 'none' : 'block';
        }
        if (emailInput) {
            emailInput.placeholder = role === 'admin' ? 'admin@storyvault.com' : 'name@example.com';
        }
    ...
    // Auto-detect role from URL parameters on page load (e.g. login.html?role=admin)
    try {
        const urlParams = new URLSearchParams(window.location.search);
        const roleParam = urlParams.get('role') || urlParams.get('portal');
        if (roleParam && ['member', 'admin'].includes(roleParam)) {
            document.addEventListener('DOMContentLoaded', () => window.setLoginRole(roleParam));
            setTimeout(() => window.setLoginRole(roleParam), 20);
        }
    } catch (e) {}
    ...
    window.handleSocialLogin = (provider) => {
        const providers = {
            ...
            apple: { name: 'Apple Member', email: 'apple.member@storyvault.local' }
        };
        ...
        showToast('Sign In Demo', `${providerLabel} sign-in was confirmed. This page stays open; dashboards are available separately as public demos.`, 'success');
    };
    ...
    window.handlePortalLogin = (event) => {
        ...
        emailInput?.classList.remove('is-invalid');
        passwordInput?.classList.remove('is-invalid');
        showToast('Sign In Verified', `Welcome ${authenticatedUser.name}. Your credentials are valid. This page will remain open.`, 'success');
    };
    ...
    window.registerMember = (event) => {
        ...
        showToast('Account Created', 'Your member account was created successfully. This page will remain open; use Sign In whenever you want to verify the account.', 'success');
    };
    ...
    const initMemberMobileDrawer = () => {
        ...
        // Beginner-friendly dismissal: backdrop, Escape, Close and selecting a menu link all work.
        ...
        sidebar.querySelectorAll('a[href]').forEach(link => link.addEventListener('click', () => {
            if (!window.matchMedia('(min-width: 992px)').matches) closeDrawer();
        }));
        document.addEventListener('keydown', (event) => {
            if (event.key === 'Escape' && body.classList.contains('dashboard-drawer-open')) closeDrawer();
        });
    ...
    const STORYVAULT_BOOKS = [
        ...
        {
            id: 'b11',
            title: 'The Starlight Alchemist',
            author: 'Aria Montgomery',
            genre: 'Fantasy',
            genreLabel: 'Fantasy / Celestial',
            rating: 4.9,
            reviewsCount: 218,
            price: '₹729',
            priceVal: 729,
            ...
            publisher: 'StoryVault Editions',
            format: 'Hardcover Collector\'s Edition',
            summary: 'An apprentice celestial alchemist in a city suspended among the clouds deciphers an ancient astral atlas that foretells a cosmic convergence.',
            description: 'High above the mist-covered peaks of Aethelgard, inside a sprawling brass-domed observatory, apprentice alchemist Aria Montgomery stumbles upon the Star Core—a forbidden manuscript written in shimmering celestial ink. When a rare alignment of twin comets begins to fracture the astral barrier, Aria must master lost stellar transmutations to prevent eternal twilight from swallowing the kingdom.',
            authorBio: 'Aria Montgomery is an award-winning epic fantasy author and worldbuilder acclaimed for rich magic systems and celestial mythology.'
        },
        {
            id: 'b12',
            title: 'The Botanist of Mars',
            author: 'Dr. Evelyn Zhao',
            genre: 'Sci-Fi',
            genreLabel: 'Sci-Fi & Astronomy',
            rating: 4.8,
            reviewsCount: 176,
            price: '₹679',
            priceVal: 679,
            ...
            isbn: '978-0-919-6612',
            pages: 416,
            publisher: 'StoryVault Frontier',
            format: 'Hardcover',
            summary: 'A solitary astrobiologist cultivates the first terraced greenhouse beneath a Martian dome, only to discover an ancient subterranean organism communicating through bioluminescence.',
            description: 'Isolated in the red rust dunes of the Martian Elysium Planitia, astrobiologist Dr. Evelyn Zhao tends to humanity\'s most ambitious experiment: a towering geodesic greenhouse sheltering Earth\'s rarest botanical specimens. But when deep subterranean roots penetrate an ancient geothermal fissure, a dormant alien bioluminescent flora awakens, broadcasting complex luminous pulses across the Martian night sky.',
            authorBio: 'Dr. Evelyn Zhao is an astrobiologist and award-winning science fiction novelist whose works explore interplanetary ecology and deep space survival.'
        }
    ];

    window.STORYVAULT_BOOKS = STORYVAULT_BOOKS;
    window.getBookById = (id) => STORYVAULT_BOOKS.find(b => b.id === id || b.id === 'b' + id || String(b.id).toLowerCase() === String(id).toLowerCase()) || STORYVAULT_BOOKS[0];

    // Seed local storage with complete catalog if empty or outdated
    try {
        const storedRaw = localStorage.getItem(STORAGE_KEYS.BOOKS_CATALOG);
        if (!storedRaw) {
            ...
            if (Array.isArray(parsed)) {
                ...
                if (updated) {
                    localStorage.setItem(STORAGE_KEYS.BOOKS_CATALOG, JSON.stringify(parsed));
                }
            }
        }
    } catch (_) {
    ...
    const initBookDetailsPage = () => {
        ...
        let rawId = params.get('id') || document.body.dataset.bookId || 'b1';
        ...
        if (borrowBtn) {
            borrowBtn.onclick = () => openBorrowCheckout(book.id);
        }

        const buyBtn = document.getElementById('book-detail-buy-btn');
        if (buyBtn) {
            buyBtn.onclick = () => openBuyCheckout(book.id);
        }
        ...
        const reviewsTabPane = document.getElementById('reviews-pane');
        if (reviewsTabPane) {
            reviewsTabPane.innerHTML = `
                <div class="mb-3 border-bottom pb-2">
                    <div class="d-flex justify-content-between">
                        <strong class="text-primary">Marcus H.</strong>
                        <span class="text-warning small"><i class="fa-solid fa-star"></i> 5.0</span>
                    </div>
                    <p class="small text-muted mb-0">"An utterly spellbinding read. The imagery and narrative in ${book.title} felt so realistic and captivating!"</p>
                </div>
                <div class="mb-2">
                    <div class="d-flex justify-content-between">
                        <strong class="text-primary">Kavya S.</strong>
                        <span class="text-warning small"><i class="fa-solid fa-star"></i> 4.9</span>
                    </div>
                    <p class="small text-muted mb-0">"Couldn't put it down. The storytelling by ${book.author} is truly exceptional. Highly recommended to StoryVault members!"</p>
                </div>`;
        }
    ...
    // 7. Action Feedback Notification System
    // Important actions (borrow, submit, renew, save, sign in/out, etc.) provide
    // visible feedback. Theme and RTL/LTR toggles stay intentionally silent.
    window.showToast = (title, message, type = 'success', options = {}) => {
        let container = document.querySelector('.toast-container-custom');
        if (!container) {
            container = document.createElement('div');
            container.className = 'toast-container-custom';
            container.setAttribute('aria-live', type === 'danger' ? 'assertive' : 'polite');
        ...
        const safeType = ['success', 'info', 'warning', 'danger'].includes(type) ? type : 'info';
        const toast = document.createElement('div');
        toast.className = `custom-toast toast-${safeType}`;
        toast.setAttribute('role', safeType === 'danger' ? 'alert' : 'status');

        const icon = document.createElement('span');
        icon.className = `custom-toast-icon toast-icon-${safeType}`;
        icon.innerHTML = `<i class="fa-solid ${iconMap[safeType]}"></i>`;

        const copy = document.createElement('div');
        copy.className = 'custom-toast-copy';
        const heading = document.createElement('strong');
        heading.className = 'custom-toast-title';
        heading.textContent = title || 'StoryVault';
        const body = document.createElement('span');
        body.className = 'custom-toast-message';
        body.textContent = message || '';
        copy.append(heading, body);

        const actions = document.createElement('div');
        actions.className = 'custom-toast-actions';
        if (options?.label && options?.href) {
            const action = document.createElement('a');
            action.className = 'custom-toast-action';
        ...
        const duration = Math.max(2500, Number(options?.duration) || 5200);
        window.setTimeout(() => {
    ...
    window.storyVaultGoBack = (fallback = 'index.html') => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const currentUser = () => {
        try {
            const stored = JSON.parse(localStorage.getItem(LIVE_KEYS.USER) || 'null');
            if (stored) return stored;
        } catch (_) {}
        const page = (window.location.pathname.split('/').pop() || '').toLowerCase();
        if (page === 'admin-dashboard.html') return { ...DEMO.admin, dashboardAccess:true, isDemo:true };
        if (page === 'member-dashboard.html') return { ...DEMO.member, dashboardAccess:true, isDemo:true };
        return null;
    };
    ...
    const normalizedCatalog = () => {
        ...
        if (!Array.isArray(catalog) || !catalog.length) {
            catalog = Array.isArray(window.STORYVAULT_BOOKS) ? window.STORYVAULT_BOOKS.slice() : [];
        } else if (Array.isArray(window.STORYVAULT_BOOKS)) {
            const existingIds = new Set(catalog.map(b => b.id));
            window.STORYVAULT_BOOKS.forEach(b => {
                if (!existingIds.has(b.id)) {
                    catalog.push({ ...b });
                }
            });
        ...
        catalog = catalog.map((book, index) => {
            ...
            const priceVal = Number.isFinite(Number(book.priceVal)) && Number(book.priceVal) > 0 ? Number(book.priceVal) : (INR_MAP[id] ?? 599);
    ...
    // ---------- Authentication override: popup-only public auth + public demo dashboards ----------
    // Normal registration/sign-in stays on the auth page and only shows feedback.
    // Dashboards are separate public demo pages with example profiles and do not need a session.
    ...
    window.registerMember = event => {
        ...
        if (registered.some(u => String(u.email).toLowerCase() === email) || [DEMO.member.email, DEMO.admin.email].includes(email)) {
            showToast('Account Exists', 'An account with this email already exists. Please use the Sign In page.', 'warning');
            return;
        }
        const profile = {
        ...
        localStorage.removeItem(LIVE_KEYS.USER); // registration must not auto-login or expose Profile
        notifyLive('member-created');
        form.reset();
        form.classList.remove('was-validated');
        showToast('Account Created', 'Your StoryVault member account was created successfully. No page redirect was performed. Use Sign In when you are ready.', 'success', { label:'Open Sign In', href:'login.html', duration:8000 });
    };
    ...
    window.handlePortalLogin = event => {
        ...
        if (role === 'admin') {
            ...
            if (email === admin.email && password === admin.password) match = admin;
        } else {
            const registered = parseJSON(LIVE_KEYS.REGISTERED, []);
            if (Array.isArray(registered)) match = registered.find(u => String(u.email).toLowerCase() === email && u.password === password) || null;
            if (!match && email === DEMO.member.email && password === DEMO.member.password) match = DEMO.member;
        }
        ...
        emailInput?.classList.remove('is-invalid');
        passwordInput?.classList.remove('is-invalid');
        const error = document.getElementById('loginError'); if (error) error.style.display='none';
        const sessionUser = { ...match, role: role === 'admin' ? 'admin' : 'member', dashboardAccess:true };
        window.setCurrentUser?.(sessionUser);
        window.syncAuthenticatedNavigation?.();
        showToast('Sign In Verified', `Welcome ${match.name}. Your credentials are valid. This page stays open; dashboards can also be viewed directly without login.`, 'success', { duration:8000 });
    };

    window.handleSocialLogin = provider => {
        const label = provider === 'google' ? 'Google' : (provider === 'apple' ? 'Apple' : provider.charAt(0).toUpperCase() + provider.slice(1));
        localStorage.removeItem(LIVE_KEYS.USER);
        showToast(`${label} Sign In`, `${label} sign-in demo completed. This page stays open; dashboards can be viewed directly without login.`, 'success', { duration:7000 });
    };
    ...
    // ---------- Borrow confirmation (loan flow; no purchase payment) ----------
    const ensureBorrowCheckoutModal = () => {
        let modal = document.getElementById('borrowCheckoutModal');
        if (modal) return modal;
        modal = document.createElement('div');
        modal.className = 'modal fade';
        modal.id = 'borrowCheckoutModal';
        modal.tabIndex = -1;
        modal.setAttribute('aria-hidden', 'true');
        modal.innerHTML = `
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content story-checkout-modal">
                    <div class="modal-header border-0 pb-0">
                        <div>
                            <span class="checkout-kicker"><i class="fa-solid fa-book-bookmark"></i> Library Loan</span>
                            <h4 class="modal-title mt-2 mb-1">Confirm Borrow Request</h4>
                            <p class="text-muted small mb-0">Borrowing is separate from purchasing and does not require payment.</p>
                        </div>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body pt-3">
                        <div class="checkout-book-summary">
                            <div class="checkout-book-icon"><i class="fa-solid fa-book-open"></i></div>
                            <div class="min-w-0">
                                <small class="text-muted d-block">Selected title</small>
                                <strong id="borrowCheckoutTitle" class="d-block text-truncate">Book</strong>
                            </div>
                        </div>
                        <div class="borrow-terms-grid mt-3">
                            <div class="borrow-term"><i class="fa-regular fa-calendar"></i><span><small>Loan period</small><strong>14 days</strong></span></div>
                            <div class="borrow-term"><i class="fa-solid fa-rotate"></i><span><small>Renewals</small><strong>Up to 2</strong></span></div>
                            <div class="borrow-term"><i class="fa-solid fa-indian-rupee-sign"></i><span><small>Payment</small><strong>No payment</strong></span></div>
                        </div>
                        <div class="checkout-note mt-3"><i class="fa-solid fa-circle-info"></i><span>Your request will be sent to the librarian for approval. Track its status from the Member Dashboard.</span></div>
                    </div>
                    <div class="modal-footer border-0 pt-0 checkout-footer">
                        <button type="button" class="btn btn-story btn-story-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" class="btn btn-story btn-story-primary" id="confirmBorrowCheckoutBtn"><i class="fa-solid fa-paper-plane me-1"></i> Send Borrow Request</button>
                    </div>
                </div>
            </div>`;
    ...
    window.openBorrowCheckout = idOrTitle => {
        const user = currentUser();
        if (!user || user.role !== 'member') {
            requestBorrow(idOrTitle);
            return;
        }
        ...
        const modalEl = ensureBorrowCheckoutModal();
        const title = modalEl.querySelector('#borrowCheckoutTitle');
        if (title) title.textContent = book.title;
        const confirm = modalEl.querySelector('#confirmBorrowCheckoutBtn');
    ...
    window.requestBorrow = idOrTitle => {
        const user = currentUser();
        if (!user || user.role !== 'member') {
            showToast('Member Sign In Required', 'Please sign in or create a member account before borrowing a book.', 'warning', { label: 'Sign In', href: 'login.html?role=member', duration: 6500 });
            return;
        }
        const book = getLiveBook(idOrTitle) || getLiveCatalog().find(b => b.title === idOrTitle);
        if (!book) { showToast('Book Not Found', 'This title could not be found in the current catalog.', 'danger'); return; }
        ...
        if (existing) { showToast('Request Already Pending', `Your request for “${book.title}” is already waiting for approval.`, 'info', { label: 'View Request', href: 'member-dashboard.html#request-status-sec', duration: 6500 }); return; }
        if (memberBorrowed(email).some(b => b.bookId === book.id)) { showToast('Already Borrowed', `“${book.title}” is already on your shelf.`, 'info', { label: 'My Books', href: 'member-dashboard.html#currently-borrowed-sec', duration: 6500 }); return; }
        const req = {
        ...
        showToast('Borrow Request Submitted', `“${book.title}” was sent for librarian approval. You can track the request from your member dashboard.`, 'success', { label: 'View Dashboard', href: 'member-dashboard.html#request-status-sec', duration: 7000 });
    };
    ...
    // ---------- Demo purchase checkout + orders ----------
    // Payment UI is intentionally frontend-only. No real payment is processed and
    // card/UPI details are never stored. A future gateway can replace submitDemoPurchase().
    const PAYMENT_LABELS = { cash: 'Cash', card: 'Card', upi: 'UPI' };

    const ensureBuyCheckoutModal = () => {
        let modal = document.getElementById('buyCheckoutModal');
        if (modal) return modal;
        modal = document.createElement('div');
        modal.className = 'modal fade';
        modal.id = 'buyCheckoutModal';
        modal.tabIndex = -1;
        modal.setAttribute('aria-hidden', 'true');
        modal.innerHTML = `
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content story-checkout-modal">
                    <div class="modal-header border-0 pb-0">
                        <div>
                            <span class="checkout-kicker"><i class="fa-solid fa-bag-shopping"></i> Demo Checkout</span>
                            <h4 class="modal-title mt-2 mb-1">Buy Edition</h4>
                            <p class="text-muted small mb-0">Choose a payment mode. This demo does not process or store real payment details.</p>
                        </div>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body pt-3">
                        <div class="checkout-book-summary">
                            <div class="checkout-book-icon"><i class="fa-solid fa-book"></i></div>
                            <div class="flex-grow-1 min-w-0">
                                <small class="text-muted d-block">Selected edition</small>
                                <strong id="buyCheckoutTitle" class="d-block text-truncate">Book</strong>
                            </div>
                            <div class="checkout-price text-end"><small class="text-muted d-block">Total</small><strong id="buyCheckoutPrice">₹0</strong></div>
                        </div>

                        <div class="mt-4">
                            <label class="form-label fw-semibold mb-2">Payment mode</label>
                            <div class="payment-method-grid" id="paymentMethodGrid">
                                <button type="button" class="payment-method-card active" data-payment-method="cash" aria-pressed="true">
                                    <span class="payment-method-icon"><i class="fa-solid fa-money-bill-wave"></i></span>
                                    <span><strong>Cash</strong><small>Pay on pickup / delivery</small></span>
                                    <i class="fa-solid fa-circle-check payment-check"></i>
                                </button>
                                <button type="button" class="payment-method-card" data-payment-method="card" aria-pressed="false">
                                    <span class="payment-method-icon"><i class="fa-regular fa-credit-card"></i></span>
                                    <span><strong>Card</strong><small>Demo card payment</small></span>
                                    <i class="fa-solid fa-circle-check payment-check"></i>
                                </button>
                                <button type="button" class="payment-method-card" data-payment-method="upi" aria-pressed="false">
                                    <span class="payment-method-icon"><i class="fa-solid fa-mobile-screen-button"></i></span>
                                    <span><strong>UPI</strong><small>Demo UPI payment</small></span>
                                    <i class="fa-solid fa-circle-check payment-check"></i>
                                </button>
                            </div>
                        </div>

                        <div class="payment-detail-panel mt-3" data-payment-panel="cash">
                            <div class="checkout-note"><i class="fa-solid fa-hand-holding-dollar"></i><span>Cash selected. Pay the displayed amount when you collect the book or receive delivery.</span></div>
                        </div>

                        <div class="payment-detail-panel mt-3 d-none" data-payment-panel="card">
                            <div class="row g-3">
                                <div class="col-12"><label class="form-label small fw-semibold">Name on card</label><input class="form-control" id="demoCardName" autocomplete="off" placeholder="Demo User"></div>
                                <div class="col-12"><label class="form-label small fw-semibold">Card number</label><input class="form-control" id="demoCardNumber" inputmode="numeric" maxlength="19" autocomplete="off" placeholder="4242 4242 4242 4242"></div>
                                <div class="col-7"><label class="form-label small fw-semibold">Expiry</label><input class="form-control" id="demoCardExpiry" maxlength="5" autocomplete="off" placeholder="12/28"></div>
                                <div class="col-5"><label class="form-label small fw-semibold">CVV</label><input class="form-control" id="demoCardCvv" inputmode="numeric" maxlength="3" autocomplete="off" placeholder="123"></div>
                            </div>
                        </div>

                        <div class="payment-detail-panel mt-3 d-none" data-payment-panel="upi">
                            <label class="form-label small fw-semibold">UPI ID</label>
                            <div class="input-group"><span class="input-group-text"><i class="fa-solid fa-at"></i></span><input class="form-control" id="demoUpiId" autocomplete="off" placeholder="name@bank"></div>
                        </div>

                        <div class="demo-payment-disclaimer mt-3"><i class="fa-solid fa-shield-halved"></i><span><strong>Demo only.</strong> No bank, card or UPI transaction is made. Payment data entered here is not saved.</span></div>
                    </div>
                    <div class="modal-footer border-0 pt-0 checkout-footer">
                        <button type="button" class="btn btn-story btn-story-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" class="btn btn-story btn-story-primary" id="confirmDemoPurchaseBtn"><i class="fa-solid fa-lock me-1"></i> Confirm Demo Order</button>
                    </div>
                </div>
            </div>`;
        document.body.appendChild(modal);

        modal.querySelectorAll('[data-payment-method]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                modal.querySelectorAll('[data-payment-panel]').forEach(panel => panel.classList.toggle('d-none', panel.dataset.paymentPanel !== method));
            });
        });
        return modal;
    };

    window.openBuyCheckout = bookId => {
        const user=currentUser();
        if(!user||user.role!=='member'){
            showToast('Member Sign In Required','Please sign in before purchasing a book.','warning',{label:'Sign In',href:'login.html?role=member',duration:6500});
            return;
        }
        const book=getLiveBook(bookId);
        if(!book){showToast('Book Not Found','This title could not be found in the current catalog.','danger');return;}
        const modal=ensureBuyCheckoutModal();
        modal.dataset.bookId=book.id;
        modal.dataset.paymentMethod='cash';
        const title=modal.querySelector('#buyCheckoutTitle'); if(title) title.textContent=book.title;
        const price=modal.querySelector('#buyCheckoutPrice'); if(price) price.textContent=money(book.priceVal);
        modal.querySelectorAll('[data-payment-method]').forEach(item=>{
            const active=item.dataset.paymentMethod==='cash';
            item.classList.toggle('active',active);item.setAttribute('aria-pressed',active?'true':'false');
        });
        modal.querySelectorAll('[data-payment-panel]').forEach(panel=>panel.classList.toggle('d-none',panel.dataset.paymentPanel!=='cash'));
        ['demoCardName','demoCardNumber','demoCardExpiry','demoCardCvv','demoUpiId'].forEach(id=>{const input=modal.querySelector('#'+id);if(input)input.value='';});
        const confirm=modal.querySelector('#confirmDemoPurchaseBtn');
        if(confirm) confirm.onclick=()=>submitDemoPurchase(modal);
        bootstrap.Modal.getOrCreateInstance(modal).show();
    };

    const submitDemoPurchase = modal => {
        const method=modal.dataset.paymentMethod||'cash';
        if(method==='card'){
            const name=(modal.querySelector('#demoCardName')?.value||'').trim();
            const number=(modal.querySelector('#demoCardNumber')?.value||'').replace(/\D/g,'');
            const expiry=(modal.querySelector('#demoCardExpiry')?.value||'').trim();
            const cvv=(modal.querySelector('#demoCardCvv')?.value||'').replace(/\D/g,'');
            if(!name||number.length<12||!/^(0[1-9]|1[0-2])\/\d{2}$/.test(expiry)||cvv.length!==3){
                showToast('Check Card Details','Enter a demo card name, valid demo card number, MM/YY expiry and 3-digit CVV.','warning');return;
            }
        }
        if(method==='upi'){
            const upi=(modal.querySelector('#demoUpiId')?.value||'').trim();
            if(!/^[A-Za-z0-9._-]{2,}@[A-Za-z0-9.-]{2,}$/.test(upi)){
                showToast('Check UPI ID','Enter a demo UPI ID such as name@bank.','warning');return;
            }
        }
        const bookId=modal.dataset.bookId;
        bootstrap.Modal.getOrCreateInstance(modal).hide();
        createBookOrder(bookId,method);
    };

    window.createBookOrder = (bookId,paymentMethod='cash') => {
        const user=currentUser(); if(!user||user.role!=='member'){showToast('Member Sign In Required','Please sign in before placing a bookstore order.','warning');return;}
        const book=getLiveBook(bookId); if(!book)return;
        const method=PAYMENT_LABELS[paymentMethod]?paymentMethod:'cash';
        const paymentLabel=PAYMENT_LABELS[method];
        const paymentStatus=method==='cash'?'Pay on Pickup / Delivery':'Demo Paid';
        const orders=parseJSON(LIVE_KEYS.ORDERS,[]);
        const order={id:`ORD-${String(Date.now()).slice(-6)}`,memberEmail:userKey(user),customer:user.name,bookId:book.id,bookTitle:book.title,date:todayISO(),amount:Number(book.priceVal),paymentMethod:paymentLabel,paymentStatus,status:'Placed',isDemo:true,createdAt:new Date().toISOString()};
        orders.unshift(order); saveJSON(LIVE_KEYS.ORDERS,orders); notifyLive('new-order'); renderAdminDashboardLive();
        const message=method==='cash'
            ? `“${book.title}” is reserved for ${money(book.priceVal)}. Cash can be paid at pickup or delivery.`
            : `Demo ${paymentLabel} payment approved for ${money(book.priceVal)}. No real transaction was made.`;
        showToast(method==='cash'?'Order Confirmed':'Demo Payment Successful',message,'success',{label:'Browse More Books',href:'books.html',duration:7500});
    };
    ...
    // ---------- Member borrowed book details modal handler ----------
    window.showBorrowedBookDetails = loanId => {
        const user = currentUser(); if (!user || user.role !== 'member') return;
        const loans = memberBorrowed(user.email);
        const loan = loans.find(l => l.id === loanId);
        if (!loan) return;

        const modalEl = document.getElementById('borrowedBookDetailsModal');
        if (!modalEl) return;

        const now = new Date();
        const dueObj = new Date((loan.dueDate || todayISO()) + 'T23:59:59');
        const diffDays = Math.ceil((dueObj - now) / 86400000);

        const nextDueObj = new Date((loan.dueDate || todayISO()) + 'T00:00:00');
        nextDueObj.setDate(nextDueObj.getDate() + 14);
        const nextDueDateStr = nextDueObj.toISOString().slice(0, 10);

        const coverEl = document.getElementById('loanDetailCover'); if (coverEl) coverEl.src = loan.cover || 'assets/images/books/book1.jpg';
        const titleEl = document.getElementById('loanDetailTitle'); if (titleEl) titleEl.textContent = loan.title;
        const authorEl = document.getElementById('loanDetailAuthor'); if (authorEl) authorEl.textContent = `By ${loan.author || 'StoryVault Author'}`;
        const idEl = document.getElementById('loanDetailId'); if (idEl) idEl.textContent = `#${loan.id}`;
        const bDateEl = document.getElementById('loanDetailBorrowedDate'); if (bDateEl) bDateEl.textContent = dateLabel(loan.borrowedDate);
        const dDateEl = document.getElementById('loanDetailDueDate'); if (dDateEl) dDateEl.textContent = dateLabel(loan.dueDate);

        const timeLeftEl = document.getElementById('loanDetailTimeLeft');
        if (timeLeftEl) {
            timeLeftEl.textContent = diffDays > 0 ? `${diffDays} days remaining` : (diffDays === 0 ? 'Due Today' : `Overdue by ${Math.abs(diffDays)} days`);
            timeLeftEl.className = diffDays <= 3 ? 'fw-bold text-danger' : 'fw-bold text-success';
        }

        const badgeEl = document.getElementById('loanDetailBadge');
        if (badgeEl) {
            badgeEl.className = diffDays < 0 ? 'badge bg-danger mb-2' : (diffDays <= 3 ? 'badge bg-warning text-dark mb-2' : 'badge bg-success mb-2');
            badgeEl.textContent = diffDays < 0 ? 'Overdue Loan' : (diffDays <= 3 ? 'Due Soon' : 'Active Loan');
        }

        const nextDueText = document.getElementById('loanDetailNextDueDateText');
        if (nextDueText) {
            nextDueText.innerHTML = `Extends due date to <strong>${dateLabel(nextDueDateStr)}</strong> (+14 Days)`;
        }

        const renewalsBadge = document.getElementById('loanDetailRenewalsLeft');
        if (renewalsBadge) {
            renewalsBadge.textContent = `${loan.renewalsLeft || 0} renewals remaining`;
        }

        const renewBtn = document.getElementById('loanDetailRenewBtn');
        if (renewBtn) {
            renewBtn.disabled = Number(loan.renewalsLeft) <= 0;
            renewBtn.onclick = () => {
    ...
    function renderMemberDashboardLive(){
        ...
        const tbody=document.getElementById('member-borrowed-list'); if(tbody){tbody.innerHTML=loans.length?loans.map(l=>{const diff=Math.ceil((new Date(l.dueDate+'T23:59:59')-now)/86400000);const badge=diff<0?'<span class="badge bg-danger">Overdue</span>':(diff<=3?'<span class="badge bg-warning text-dark">Due Soon</span>':'<span class="badge bg-success">Active</span>');return `<tr><td><div class="d-flex align-items-center gap-3 cursor-pointer" onclick="showBorrowedBookDetails('${esc(l.id)}')" title="Click to view due date & loan details"><img src="${esc(l.cover)}" alt="${esc(l.title)}" style="width:42px;height:60px;object-fit:cover;border-radius:4px" class="shadow-sm"><div><h6 class="mb-0 fw-bold text-primary-hover">${esc(l.title)}</h6><small class="text-muted">${esc(l.author)}</small></div></div></td><td>${dateLabel(l.borrowedDate)}</td><td><span class="badge bg-warning-subtle text-dark border border-warning px-2 py-1"><i class="fa-solid fa-calendar-day me-1 text-warning"></i><strong class="text-dark">${dateLabel(l.dueDate)}</strong></span></td><td><span class="${diff<=3?'fw-bold text-danger':'fw-semibold'}">${diff>0?`${diff} days left`:diff===0?'Due today':`Overdue (${Math.abs(diff)}d)`}</span></td><td>${badge}</td><td><div class="d-flex gap-1"><button class="btn btn-sm btn-story-secondary" onclick="showBorrowedBookDetails('${esc(l.id)}')" title="View complete due date details"><i class="fa-solid fa-circle-info me-1"></i>Details</button><button class="btn btn-sm btn-story-outline" onclick="renewBook('${esc(l.id)}')" ${Number(l.renewalsLeft)<=0?'disabled':''} title="Extend loan by +14 days"><i class="fa-solid fa-rotate-right me-1"></i>Renew (${Number(l.renewalsLeft)||0})</button></div></td></tr>`;}).join(''):'<tr><td colspan="6" class="text-center py-5 text-muted"><i class="fa-solid fa-book-open fs-3 d-block mb-2"></i>No active borrowings yet. Borrow a title and it will appear here after admin approval.</td></tr>';}
    ...
    // ---------- Edit Catalog Book Handler & Modal Launcher ----------
    window.openEditBookModal = (bookId) => {
        const admin = currentUser();
        if (!admin || admin.role !== 'admin') {
            showToast('Admin Access Required', 'Please sign in with administrator credentials to edit catalog titles.', 'warning');
            return;
        }

        const book = getLiveBook(bookId);
        if (!book) {
            showToast('Book Not Found', 'Could not locate target book in the catalog.', 'danger');
            return;
        }
        ...
        const idInput = document.getElementById('editBookId'); if (idInput) idInput.value = book.id;
        const titleInput = document.getElementById('editBookTitle'); if (titleInput) titleInput.value = book.title || '';
        const authorInput = document.getElementById('editBookAuthor'); if (authorInput) authorInput.value = book.author || '';
        
        const genreSelect = document.getElementById('editBookGenre');
        if (genreSelect) {
            const currentGenre = book.genre || book.genreLabel || 'Fiction';
            genreSelect.value = Array.from(genreSelect.options).some(o => o.value === currentGenre) ? currentGenre : 'Fiction';
        }

        const isbnInput = document.getElementById('editBookIsbn'); if (isbnInput) isbnInput.value = book.isbn || '978-0-321-7890';
        const copiesInput = document.getElementById('editBookCopies'); if (copiesInput) copiesInput.value = Number.isFinite(Number(book.copies)) ? Number(book.copies) : 1;
        const priceInput = document.getElementById('editBookPrice'); if (priceInput) priceInput.value = book.priceVal || 599;
        const coverInput = document.getElementById('editBookCover'); if (coverInput) coverInput.value = book.cover || 'assets/images/books/book1.jpg';
        const summaryInput = document.getElementById('editBookSummary'); if (summaryInput) summaryInput.value = book.summary || book.description || '';

        bootstrap.Modal.getOrCreateInstance(modalEl).show();
    };

    window.handleAdminSaveBookEdit = (event) => {
        event.preventDefault();
        const form = event.currentTarget;
        if (!form.checkValidity()) {
            form.classList.add('was-validated');
        ...
        let catalog = getLiveCatalog();
        const idx = catalog.findIndex(b => String(b.id) === String(bookId));
        if (idx < 0) {
            showToast('Update Failed', 'Target book record was not found.', 'danger');
            return;
        }

        const title = document.getElementById('editBookTitle')?.value.trim() || catalog[idx].title;
        const author = document.getElementById('editBookAuthor')?.value.trim() || catalog[idx].author;
        const genre = document.getElementById('editBookGenre')?.value || catalog[idx].genre;
        const isbn = document.getElementById('editBookIsbn')?.value.trim() || catalog[idx].isbn;
        const copies = Math.max(0, Number(document.getElementById('editBookCopies')?.value || 0));
        const priceVal = Math.max(0, Number(document.getElementById('editBookPrice')?.value || 0));
        const cover = document.getElementById('editBookCover')?.value.trim() || catalog[idx].cover || 'assets/images/books/book1.jpg';
        const summary = document.getElementById('editBookSummary')?.value.trim() || catalog[idx].summary;
        ...
        catalog[idx] = {
            ...
            isbn,
            copies,
            totalCopies: Math.max(copies, Number(catalog[idx].totalCopies || copies)),
            priceVal,
            price: money(priceVal),
            cover,
            summary,
            description: summary,
            stock: copies > 0 ? `${copies} Copies Left` : 'On Reserve',
            availability: copies > 0 ? 'available' : 'borrowed',
            statusLabel: copies > 0 ? 'Available' : 'Unavailable'
        };
        ...
        notifyLive('catalog-update');
        renderAdminDashboardLive();
        showToast('Book Details Updated', `“${title}” details updated in the live catalog.`, 'success');
    };

    function renderAdminActiveLoansLive() {
        const tbody = document.getElementById('admin-active-loans-tbody');
        if (!tbody) return;
        const borrowedStore = parseJSON(LIVE_KEYS.BORROWED, {});
        const members = allMemberRows();
        const allLoans = [];

        Object.keys(borrowedStore).forEach(email => {
            const memberObj = members.find(m => m.email.toLowerCase() === email.toLowerCase()) || { name: email, email };
            const loans = Array.isArray(borrowedStore[email]) ? borrowedStore[email] : [];
            loans.forEach(l => {
                allLoans.push({ ...l, memberName: memberObj.name, memberEmail: email });
            });
        });

        const countBadge = document.getElementById('admin-active-loans-count');
        if (countBadge) countBadge.textContent = `${allLoans.length} Active Loans`;

        if (allLoans.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="text-center py-4 text-muted">No active member loans found.</td></tr>';
            return;
        }

        const now = new Date();
        tbody.innerHTML = allLoans.map(l => {
            const dueObj = new Date((l.dueDate || todayISO()) + 'T23:59:59');
            const diffDays = Math.ceil((dueObj - now) / 86400000);
            const statusBadge = diffDays < 0 ? '<span class="badge bg-danger">Overdue</span>' : (diffDays <= 3 ? '<span class="badge bg-warning text-dark">Due Soon</span>' : '<span class="badge bg-success">Active</span>');

            return `
                <tr>
                    <td><code>#${esc(l.id)}</code>${l.isDemo ? '<br><small class="text-muted">Demo</small>' : ''}</td>
                    <td><strong>${esc(l.memberName)}</strong><br><small class="text-muted">${esc(l.memberEmail)}</small></td>
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            <img src="${esc(l.cover || 'assets/images/books/book1.jpg')}" alt="${esc(l.title)}" style="width:30px;height:42px;object-fit:cover" class="rounded">
                            <div><strong>${esc(l.title)}</strong><br><small class="text-muted">${esc(l.author)}</small></div>
                        </div>
                    </td>
                    <td>${dateLabel(l.borrowedDate)}</td>
                    <td>
                        <span class="badge bg-warning-subtle text-dark border border-warning px-2 py-1">
                            ...
                            <strong>${dateLabel(l.dueDate)}</strong>
                        </span>
                    </td>
                    <td>
                        <div><strong class="${diffDays <= 3 ? 'text-danger' : 'text-success'}">${diffDays > 0 ? diffDays + ' days left' : (diffDays === 0 ? 'Due Today' : `Overdue (${Math.abs(diffDays)}d)`)}</strong></div>
                        ${statusBadge}
                    </td>
                    <td>
                        <button class="btn btn-sm btn-story-secondary" onclick="returnBorrowedBook('${esc(l.id)}')">
    ...
    function renderAdminDashboardLive(){renderAdminBorrowRequestsLive();renderAdminCatalogLive();renderAdminMembersLive();renderAdminActiveLoansLive();renderAdminOrdersLive();renderAdminMetricsLive();}
    ...
    // ---------- Synchronize Live Catalog Edits Across All Pages ----------
    const syncCatalogToUI = () => {
        const catalog = getLiveCatalog();
        if (!Array.isArray(catalog) || !catalog.length) return;

        // 1. Sync Book Detail page if currently viewing book-details.html
        const detailTitle = document.getElementById('book-detail-title');
        if (detailTitle) {
            const params = new URLSearchParams(location.search);
            const id = params.get('id') || document.body.dataset.bookId || 'b1';
            const book = catalog.find(b => String(b.id) === String(id) || String(b.title).toLowerCase() === String(id).toLowerCase()) || catalog[0];
            if (book) {
                document.title = `${book.title} | StoryVault`;
                detailTitle.textContent = book.title;

                const authorEl = document.getElementById('book-detail-author');
                if (authorEl) authorEl.textContent = book.author.startsWith('By ') ? book.author : `By ${book.author}`;

                const coverEl = document.getElementById('book-detail-cover') || document.getElementById('book-detail-hero-cover');
                if (coverEl) {
                    coverEl.src = book.cover || 'assets/images/books/book1.jpg';
                    coverEl.alt = book.title;
                }

                const priceEl = document.getElementById('book-detail-price');
                if (priceEl) priceEl.textContent = money(book.priceVal);

                const stockPill = document.getElementById('book-detail-stock-pill');
                if (stockPill) {
                    stockPill.textContent = book.copies > 0 ? `${book.copies} Copies Available` : 'On Reserve';
                    stockPill.className = book.copies > 0 ? 'fw-semibold text-success' : 'fw-semibold text-warning';
                }

                const genreBadge = document.getElementById('book-detail-genre');
                if (genreBadge) genreBadge.textContent = book.genreLabel || book.genre || 'General';

                const isbnEl = document.getElementById('book-detail-isbn');
                if (isbnEl) isbnEl.textContent = book.isbn || '978-0-321-7890';

                const summaryEl = document.getElementById('book-detail-summary');
                if (summaryEl && (book.summary || book.description)) {
                    summaryEl.textContent = book.summary || book.description;
                }

                const borrowBtn = document.getElementById('book-detail-borrow-btn');
                if (borrowBtn) borrowBtn.onclick = () => openBorrowCheckout(book.id);

                const buyBtn = document.getElementById('book-detail-buy-btn');
                if (buyBtn) buyBtn.onclick = () => openBuyCheckout(book.id);
            }
        }

        // 2. Sync all book cards across books.html, index.html, home-2.html, etc.
        catalog.forEach(book => {
            const bookId = String(book.id);
            const cardElements = document.querySelectorAll(
                `[data-book-id="${bookId}"], a[href*="id=${bookId}"], .catalog-item-col[data-title="${CSS.escape(book.title)}"]`
            );

            cardElements.forEach(el => {
                const card = el.closest('.catalog-item-col, .story-card, .book-card-item, .product-card') || el;
                if (!card) return;

                card.dataset.title = book.title;
                card.dataset.genre = book.genre || book.genreLabel || 'General';
                card.dataset.price = String(book.priceVal);
                card.dataset.availability = book.copies > 0 ? 'available' : 'borrowed';

                const img = card.querySelector('img.book-cover, .book-card-img-wrap img, img.card-img-top');
                if (img) {
                    img.src = book.cover || 'assets/images/books/book1.jpg';
                    img.alt = book.title;
                }

                const titleEl = card.querySelector('.book-title, h4.book-title, h5.book-title, .product-title');
                if (titleEl) {
                ...
                const authorEl = card.querySelector('.book-author, .product-author');
                if (authorEl) {
                    authorEl.textContent = book.author.startsWith('By ') ? book.author : `By ${book.author}`;
                }

                const genreEl = card.querySelector('.book-genre, .product-genre');
                if (genreEl) {
                    genreEl.textContent = book.genreLabel || book.genre || 'General';
                }

                const priceEl = card.querySelector('.book-price, .product-price');
                if (priceEl) {
                    priceEl.textContent = money(book.priceVal);
                }

                const statusBadge = card.querySelector('.book-badge-status, .badge-status');
                if (statusBadge) {
                    statusBadge.textContent = book.copies > 1 ? `${book.copies} Copies Left` : (book.copies === 1 ? '1 Copy Left' : 'On Reserve');
                    statusBadge.className = `badge badge-glow book-badge-status ${book.copies > 1 ? 'bg-success' : (book.copies === 1 ? 'bg-warning text-dark' : 'bg-secondary')}`;
                }

                card.querySelectorAll('a.book-cover-link, a.btn-story-outline').forEach(link => {
                    link.href = `book-details.html?id=${encodeURIComponent(book.id)}`;
                });
            });
        });
    };
    window.syncCatalogToUI = syncCatalogToUI;

    renderMemberDashboardLive(); renderAdminDashboardLive(); syncCatalogToUI(); refreshWishlist();

    window.addEventListener('storage', event => {
        if([LIVE_KEYS.REQUESTS,LIVE_KEYS.BORROWED,LIVE_KEYS.HISTORY,LIVE_KEYS.ORDERS,LIVE_KEYS.REGISTERED,LIVE_KEYS.CATALOG].includes(event.key)){
            renderMemberDashboardLive(); renderAdminDashboardLive(); syncCatalogToUI();
        }
    });
