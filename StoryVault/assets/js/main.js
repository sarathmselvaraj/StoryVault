...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 7. Global notification policy
    // StoryVault uses silent UI state changes for theme, layout and dashboard actions.
    // Popup/toast alerts are intentionally disabled across every page so controls
    // never cover content or distract users on mobile, tablet or desktop.
    window.showToast = () => false;

    // Defensive cleanup in case a stale toast from an older cached build exists.
    document.querySelectorAll('.toast-container-custom, .custom-toast').forEach((node) => node.remove());
    ...
    const syncCatalogToUI = () => {
        ...
        if (detailTitle) {
            ...
            if (book) {
                ...
                const borrowBtn = document.getElementById('book-detail-borrow-btn');
                if (borrowBtn) borrowBtn.onclick = () => requestBorrow(book.id);

                const buyBtn = document.getElementById('book-detail-buy-btn');
                if (buyBtn) buyBtn.onclick = () => createBookOrder(book.id);
            }
        }
